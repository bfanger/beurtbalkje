...
Therefore instead of a connection refused error while the (development) service is restarting, the connection is accepted and the client waits until it gets to send and receive data.
...
Because the client enters a queue and waits for service the project is called ["beurtbalkje"](https://nl.wikipedia.org/wiki/Beurtbalkje) which is the dutch word for [checkout divider](https://en.wikipedia.org/wiki/Checkout_divider).
